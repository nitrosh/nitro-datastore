...
class TestQuickPerformance:
    ...
    def test_5mb_dataset_operations(self):
        ...
        print(f"   ✓ 300 get() operations in {get_time:.4f}s ({300/get_time:.0f} ops/sec)")
        ...
        print(f"   ✓ 300 has() operations in {has_time:.4f}s ({300/has_time:.0f} ops/sec)")
        ...
        print(f"   ✓ 100 set() operations in {set_time:.4f}s ({100/set_time:.0f} ops/sec)")

        print("\n6. Testing query() on users...")
        start = time.time()
        dark_users = (data.query('users')
        ...
        print(f"      Keys: {stats['total_keys']}, Depth: {stats['max_depth']}, "
        ...
        total_ops_time = (get_time + has_time + set_time + query_time +
                         query_complex_time + list_paths_time + find_paths_time +
                         find_keys_time + stats_time + export_time + bulk_time +
                         merge_time + flatten_time)
...
class TestMemoryEfficiency:
    ...
    def test_memory_usage(self):
        ...
        start = time.time()
        for _ in range(50):
            results = data.query('users').where(lambda u: u.get('id', 0) < 100).execute()
        query_time = time.time() - start
