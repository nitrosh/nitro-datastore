...
# Get published posts
published = blog.query('posts').where(lambda p: p.get('status') == 'published').execute()
print(f"Published posts: {len(published)}")

# Get most popular posts
popular = (blog.query('posts')
...
view_increase = blog.update_where(
    condition=lambda p, v: (
        p.endswith('.views') and
        isinstance(v, (int, float)) and
        # Only published posts get views
        any(post.get('views') == v and post.get('status') == 'published'
...
analytics = NitroDataStore({
    ...
    'summary': {
        ...
        'published_posts': len([p for p in blog.posts if p.get('status') == 'published']),
