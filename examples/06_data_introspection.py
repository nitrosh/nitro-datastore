...
prefixed_data = data.transform_all(
    lambda path, value: f"BLOG: {value}" if (path.startswith('site.') and isinstance(value, str)) else value
)
...
print(f"  Access: snake_case_data.config.theme_color = {snake_case_data.config.theme_color}")
...
for i, (key, value) in enumerate(list(flat.items())[:10], 1):
    value_str = str(value) if not isinstance(value, (list, dict)) else f"<{type(value).__name__}>"
...
result = (original
    .transform_keys(lambda k: k.replace('-', '_'))  # snake_case keys
    .transform_all(lambda p, v:  # Title case product names
        v.title() if p.endswith('_name') and isinstance(v, str) else v)
    .transform_all(lambda p, v:  # Round prices
