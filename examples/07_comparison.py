...
# Nested structures
nested1 = NitroDataStore({"user": {"name": "Alice", "settings": {"theme": "dark"}}})
nested2 = NitroDataStore({"user": {"name": "Alice", "settings": {"theme": "dark"}}})
nested3 = NitroDataStore({"user": {"name": "Alice", "settings": {"theme": "light"}}})
...
new_config = NitroDataStore(
    {
        ...
        "database": {"host": "prod-db.example.com", "port": 5432},  # Changed  # Same
...
v1_1 = NitroDataStore(
    {
        ...
        "config": {"timeout": 5000, "retries": 3, "cache_ttl": 3600},  # New config
    }
)
...
expected_config = NitroDataStore(
    {
        ...
        "cache": {"enabled": True, "redis_url": "redis://prod-redis:6379"},
        "security": {"cors_enabled": True, "rate_limit": 1000},
    }
)
...
# Base document
base = NitroDataStore(
    {"document": {"title": "Project Plan", "status": "draft", "owner": "Alice"}}
)
